...
- 导入/导出配置
- 快捷键录制
> 注：以上为最初规划；目前已新增 **自定义快捷键（MVP）** 与 **长按调整顺序**（见下方“已实现”）。
...
## 已实现（MVP）
- 快捷键条末尾新增 `+`：进入 `自定义快捷键` 页面（选择系统按键/字母/数字 + 修饰键）。
- 自定义快捷键页面提供 **组合键模式（Chord）**：修饰键 + 主按键，发送不依赖输入法。
- 快捷键按钮支持长按菜单：`左移 / 右移 / 改名`（避免与 Backspace 等“长按连发”冲突）。
- 设置页“添加快捷键”入口已改为进入同一个 `自定义快捷键` 页面。

## 本地验证
- `flutter test` 全量通过（新增 `test/shortcut_order_utils_test.dart` 覆盖排序逻辑）。
